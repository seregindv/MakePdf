<!-- text below image. obsolete because of description -> lines/line -->
    ...
    <root xmlns="http://www.w3.org/1999/XSL/Format">
      ...
      <page-sequence master-reference="p0">
        <flow flow-name="xsl-region-body" font-family="Tahoma" text-align="justify">
          ...
          <!-- contents -->
          <xsl:if test="/Gallery/Contents != ''">
            <block text-indent="4mm">
              <xsl:value-of select="/Gallery/Contents" />
            </block>
          </xsl:if>
        </flow>
      </page-sequence>
      ...
      <xsl:for-each select="/Gallery/Gallery/Items/GalleryItem">
        ...
        <xsl:element name="page-sequence">
          <xsl:attribute name="master-reference">
            <xsl:choose>
              <xsl:when test="Description != ''">p0</xsl:when>
              <xsl:otherwise>
                <xsl:value-of select ="concat('p',Size/Width,'-',Size/Height)" />
              </xsl:otherwise>
            </xsl:choose>
          ...
          <flow flow-name="xsl-region-body" font-family="Tahoma" text-align="justify">
            ...
            <!--description-->
            <xsl:if test="Description != '' or last() = position()">
              <xsl:if test="Description != ''">
                <block margin-left="2mm" margin-right="2mm" font-size="5pt">
